    ...
    def _iterate(self, target):
        ...
        while np.linalg.norm(self.joints[-1] - target) > self.tolerance:
            ...
            for i in reversed(range(0, last)):
                next, curr = self.joints[i + 1], self.joints[i]
                len_s = self.lengths[i] / np.linalg.norm(next - curr)
                self.joints[i] = (1 - len_s) * next + len_s * curr

            self.joints[0] = initial_p
            for i in range(0, last):
                next, curr = self.joints[i + 1], self.joints[i]
                len_s = self.lengths[i] / np.linalg.norm(next - curr)
                self.joints[i + 1] = (1 - len_s) * curr + len_s * next
        return iteration
